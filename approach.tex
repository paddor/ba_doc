...
\paragraph{Synchronization goes both ways}
...
\paragraph{Variant 1: Sync self-subtree only.}
...
\begin{description}
	\item [Hardware failure on the active node:] \hfill\\
		...
		\begin{itemize}
			\item A non-reduntant disk can fail fatally
...
The sockets used by both HA peers to send and receive hearbeats/state
...
Imagine part of the currently active peer's network equipment fails, i.e. its \gls{NIC}
connected to the switch fails, or its switch port fails. In that case it'll be unreachable
to the rest of the federation. A failover would be desired. But it
...
wich would be zero in case it is cut off from its clients. That way, the
...
In case the clients talk to the HA cluster via \zmq sockets, and \zmq abstracts
connection handling completely away, telling how many clients are connected is
a bit less straight-forward. \zmq declares such needs as application matter.
...
the heartbeats, or it could be published via the DIM.
...
\begin{description}
	...
	\item [Continuous synchronization:]\hfill\\
		...
		is hold in the DIM. That means any node in the federation could
		do the sampling and persisting of time series data by itself.
		{\color{red}It is not clear yet, what's the best solution here.}\\
		Furthermore, in case a Roadster federation will be
		running for a very long time without interrupts, there needs to
		be a mechanism to bubble up the deletion of old time series
		records in an event-driven fashion. This can be done by simply
		signalling it as soon as they're deleted locally.
