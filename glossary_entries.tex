...
% general
\newacronym{ABC}{ABC}{Assignments, Branches, Conditions}
...
\newacronym{ZMTP}{ZMTP}{ZeroMQ Message Transport Protocol}
...
\newacronym{CSP}{CSP}{Clone State Protocol}
\newacronym{CMP}{CMP}{Case Management Protocol}
...
\newacronym{FCP}{FCP}{Federation Communication Protocol}
\newacronym{BSP}{BSP}{Binary Star Protocol}
...
\newglossaryentry{case}{
	name={alarm case},
	description={An alarm case in a Roadster application requiring human interaction}
}
...
\newglossaryentry{cztop}{
	name={CZTop},
	description={A modern, \acrshort{FFI} based \gls{ruby} binding for \gls{czmq}, authored by Patrik Wenger}
}
...
browsers and web servers, standardized by the \acrshort{IETF} as \acrshort{RFC} 6455 in
2011}
}
...
\newglossaryentry{iec-104}{
	...
	description={A \acrshort{IEC} transmission protocol used by SCADA applications in power system automation that enables communication via standard networks}
}
...
\newglossaryentry{Z85}{
	name={Z85 armor},
	description={a space efficient, \acrshort{ASCII} based, string-safe variant of the Base85 binary-to-text encoding}
}

\newglossaryentry{tc}{
	name={TokyoCabinet},
	description={a library\footnote{\url{http://fallabs.com/tokyocabinet/}} to manage a serverless key-value store in a single file}
}
...
\newglossaryentry{gherkin}{
	name={Gherkin},
	description={A simple, formal language to write feature specifications in
...
\newglossaryentry{ovs}{
	name={Open vSwitch},
	description={An implementation of a virtual multilayer switch inside the Linux kernel}
}

\newglossaryentry{event-driven}{
	name={event-driven},
	description={A programming paradigm\footnote{\url{https://en.wikipedia.org/wiki/Event-driven_programming}} where events (such as user input, sensor data, and messages) determine the program flow}
}

\newglossaryentry{shared-nothing-arch}{
	name={shared-nothing architecture},
	description={A distributed computing architecture\footnote{\url{https://en.wikipedia.org/wiki/Shared_nothing_architecture}} defined by the absence of centrally stored state, and where nodes/actors share no memory. This architecture is known to be very scalable}
}
