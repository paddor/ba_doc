...
\begin{center}
  \begin{longtable}{|p{6mm}|p{30mm}|p{6mm}|p{8mm}|p{30mm}|p{64mm}|}
    ...
    \hline
    \endlastfoot
    R01 & Roadster requires different ZMQ	& \cellcolor{yellow!50}1	& \cellcolor{yellow!50}3 	& check with client	& extract and run affected ZMQ 	\\
		& ZMQ contexts to function		 	& \cellcolor{yellow!50}		& \cellcolor{yellow!50}		& (done)			& sockets in their own process 	\\
		& (not possible with CZTop because	& \cellcolor{yellow!50}		& \cellcolor{yellow!50}		&					& 								\\
		& CZMQ hides contexts)				& \cellcolor{yellow!50}		& \cellcolor{yellow!50}		&					& delay: 1-2 days				\\ \hline
	...
	R06 & wrong time estimations  			& \cellcolor{orange!50}4 	& \cellcolor{orange!50}3 	& use time well 	& If possible, change the duration 		\\
