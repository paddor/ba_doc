...
\autoref{fig:roadster:overallsys} illustrates the typical, overall system. The acronyms
...
\subsection{\zmq}
To understand Roadster's architecture and the rest of this document, it is
...
% TODO: socket types and communication patterns
...
unmaintained and does not support recent versions of \zmq (namely the ones
...
\begin{description}
	\item [CORE:]\hfill\\
		...
		key role in keeping state in all actors synchronized. More
	...
	\item [CONSOLE:]\hfill\\
		This actor does not exist yet, but is planned. It's supposed to
...
does not mean there are no other mechanisms in place to ensure reliability.
...
replicated across all COMM actors to keep the DIM synchronized. This works by
...
% TODO: maybe use class names after all, ICANHAZ is confusing and VERY informal

% TODO CSP is bidirectional!

The existing \acrfull{CSP} is closely related to the \gls{clone-pattern} from the \gls{zguide}. Its
goal is to keep a state (a list of key-value pairs) synchronized across a set
of participants. To greatly reduce the complexity, it is not decentralized, but
uses server/client architecture. The server part serves as the authorative
source for all changes to the DIM.
...
By making all updates go through the server, a single sequence is enforced,
