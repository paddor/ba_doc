...
\begin{itemize}
	\item Ability to declare and run a Roadster application in a
hierarchical federation spanning multiple autonomous nodes
	\item Extension of existing protocols for inter-node operation
	\item High availability clustering of two nodes
	\item Secure inter-node communications (optional)
	\item HA extensions for the standardized M2M interface (OPC-UA) for client systems (optional)
\end{itemize}

% approach and technologies
% picture: technology cloud
The students gathered exact requirements, developed solutions fitting Roadster's
layered software architecture, including prototypes, new protocols and full-stack actors.
All software contributions have been made following engineering methodology and
apply the SOLID principles of OOP.
The modern high-security cryptography software
Libodium (NaCl) is used to encrypt and authenticate inter-node network
traffic as well as cryptographically sign messages to provide
end-to-end security.
	New and a significant amount of existing functionality
has been covered by comprehensive test suites ranging from unit tests to system
tests encompassing virtual networks of fully isolated Roadster nodes.
BDD frameworks and technologies such as GitLab CI, Mininet, and Docker have been used for
continuous integration and quality assurance.

% result
% picture: terminal screenshot of startup sequence
Federations of autonomous Roadster nodes (including HA clusters) can now be declared using an expressive DSL.
State is replicated across nodes through secure inter-node communication sockets.
HA clusters are run in hot-standby mode. A straight-forward integration of HA
awareness into the OPC-UA interface is prepared. Utilities to simplify
the bootstrapping process of a new Roadster application (key generation
and distribution) are provided and described.
