...
\subsubsection{Doc build}
% TODO translate
Das Erstellen dieses Dokuments, sprich LaTeX zu PDF compilieren wird auch von CI gehandhabt. 
Anschliessend wird das enstandene Artifakt, in diesem Fall ein PDF, 
mit einer neuen Release Version versehen auf dem öffentlichen Git veröffentlicht.

% TODO translate
\subsubsection{Docker-Container}
Um eine saubere Testumgebung für die Unit- und System-Tests bereitzustellen wurde indes ein Docker-Container erstellt, 
welcher die benötigten Komponenten bei jedem Test neulädt. 
Zu den Haupt-Komponenten gehört: mininet, ruby, python, roadster (die aktuellste Version, welche gepushed wurde).
...
\begin{description}
	...
	\item [Fake:]\hfill\\
		Fake \glspl{PLC} are simple simulation robots written in ruby.
\end{description}

% TODO translate
Nach jeder Iterationsphase oder Änderung im Code können die automatischen Systemtests
auf dem Gitlab Server ausgeführt werden. Die Systemtests führen den Roadster aus
und es wird anhand des Outputs überprüft ob das Feature korrekt läuft.
...
Bei jedem System-Test wird die aktuellste Version von der ba-roadster-app nachgeladen und der simultane
Branch ausgewählt, wie der zu testende, der Roadster app.
Die benötigten Konfiguration Dateien werden vor dem Test mittels eines bash scripts in die ba-roadster-app
geladen und es werden noch Ordner für das Logging erstellt.

% TODO translate
\subsubsection{Verwendete Sprache}
Die Systemtest werden mit Python geschrieben, weil die mininet Schnittstelle
für die Steuerung des Netwerks und deren Hosts mit Python geschrieben ist.

% TODO translate
\subsubsection{Feature Tests}
Um mit Python die Feature-Test im Cucumber Format verarbeiten zu können wird das Behave Framework verwendet.

% TODO translate
\subsubsubsection{Ordnerstruktur}
Die Ordnerstruktur im Docker für die zu testende App sieht wie folgt aus:
	/tmp
	  -> /roadster
	      -> /features
          -> /projects
...
% TODO translate			  
\subsubsection{Roadster Federation Aufbau}
Die Federation ist wie folgt aufgebaut.
   np-npB
  /    \ 
 sN1   sN2

Auf dem Root hat es ein HA Pärchen, welches nur im HA Feature Test 
aktiv ist und ansonsten nur ein einziger Root aktiv ist.
Im sN1 Node laufen zwei Simulation-Roboter, welche fortlaufend 
neue Cases und DIM update generieren. Der sN2 Node
ist ein stiller zuhörer und hat keine aktive Steuerungen zum überwachen.

\subsubsection{Netzplan aus Sicht von mininet}
            s1   
      /   |   |    \
   sN1  sN2  np - nPB

Alle Nodes sind an einem Switch angeschlossen und kommunizieren über diesen. 
Der root primary Node ist zudem mit dem root backup Node noch zusätzlich verbunden.
...
supernode's SUBFED actor will \cpp{bind()} its socket, and a subnode's
SUPERFED actor will \cpp{connect()} to its supernode. This is consistent
...
engine, as well as in the SUBFED engine. CSP-related client functionality has
to be present in all COMM existing engines, the SUPERFED engine, \emph{as well
as the SUBFED engine}, since it also acts as a CSP-client to the CORE engine on
the same node.
...
SUPERFED actor has to recognize this situation and discard update in question
immediately.
